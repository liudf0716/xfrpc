...
void set_cur_stream(struct tmux_stream *stream) { cur_stream = stream; }
...
void send_window_update(struct bufferevent *bout, struct tmux_stream *stream,
    ...
    // debug(LOG_DEBUG, "send window update: flags %d, stream_id %d delta %d,
    // recv_window %u length %u", 				flags, stream->id, delta, stream->recv_window,
    //length);
}
...
static int incr_send_window(struct bufferevent *bev,
    ...
    // debug(LOG_DEBUG, "incr_send_window : stream_id %d length %d send_window
    // %d", 				stream->id, length, stream->send_window);
...
int handle_tcp_mux_stream(struct tcp_mux_header *tmux_hdr,
    ...
    // debug(LOG_DEBUG, "handle_tcp_mux_stream stream_id %d type %d flags %d",
    // stream_id, tmux_hdr->type, flags);
...
uint32_t tmux_stream_write(struct bufferevent *bev, uint8_t *data,
    ...
    uint32_t max = length;
    struct bufferevent *bout = get_main_control()->connect_bev;
    // debug(LOG_DEBUG, "tmux_stream_write stream id %u: send_window %u tx_ring
    // sz %u length %u", 				stream->id, stream->send_window, tx_ring->sz, length);
